...
import { mockTrainers } from '@/lib/registryMockData';
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface/50 border-b border-border/50 text-xs text-textSecondary font-medium">
                ...
                <th className="p-4">Licence grade</th>
                <th className="p-4">Stable name</th>
            ...
            <tbody className="divide-y divide-border/50">
              {mockTrainers.map((t) => (
                <tr key={t.id} className="group hover:bg-surface/30 transition-colors">
                  <td className="p-4 font-mono text-sm">
                    <Link href={`/trainers/${t.id}`} className="hover:text-primary transition-colors block w-full h-full">
                      {t.id}
                    </Link>
                  </td>
                  ...
                  <td className="p-4 text-sm">{t.licenseGrade}</td>
                  <td className="p-4 text-sm">{t.stables}</td>
                  <td className="p-4 text-sm text-primary">{t.licensingClub}</td>
                  <td className="p-4 text-sm">
                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${t.status === 'Active' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                      {t.status}
                    </span>
                  </td>
                </tr>

          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface/50 border-b border-border/50 text-xs text-textSecondary font-medium">
                ...
                <th className="p-4">Registered colours</th>
                <th className="p-4">Home club</th>
                <th className="p-4">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/50">
              {mockOwners.map((o) => (
                <tr key={o.id} className="group hover:bg-surface/30 transition-colors">
                  <td className="p-4 font-mono text-sm">
                    <Link href={`/owners/${o.id}`} className="hover:text-primary transition-colors block w-full h-full">
                      {o.id}
                    </Link>
                  </td>
                  ...
                  <td className="p-4 text-sm">{o.type}</td>
                  <td className="p-4 text-sm">{o.registeredColours}</td>
                  <td className="p-4 text-sm text-primary">{o.homeClub}</td>
                  <td className="p-4 text-sm">
                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${o.status === 'Active' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                      {o.status}
                    </span>
                  </td>
                </tr>
